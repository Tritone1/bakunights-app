...
The homepage points wheel is account-backed rather than a client-side deal picker. A spin unlocks only after a merchant verifies the customer's in-app GS QR/code during an in-store visit. Each verified redemption can fund exactly one spin, and each customer can use at most one spin per Baku calendar day. Both rules are enforced by database uniqueness constraints, so refreshing the page, making simultaneous requests, or using another device cannot reuse a visit or bypass the daily limit. Additional verified, unused visits remain available for later days.
