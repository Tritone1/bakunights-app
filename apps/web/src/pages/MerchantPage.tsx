...
function MenuManager({ venues, categories, items, onChanged }: { venues: ManagedVenue[]; categories: MenuCategory[]; items: MenuItem[]; onChanged: () => Promise<void> }) {
  ...
  return <section className="mt-6">
    ...
    <div className="mt-5 space-y-5">{categories.map((category) => { const group = venueItems.filter((item) => item.categoryId === category.id); return group.length ? <div key={category.id}><h3 className="mb-2 text-sm font-bold uppercase tracking-[.16em] text-white/45">{category.name}</h3><div className="grid gap-2 md:grid-cols-2">{group.map((item) => <article key={item.id} className={`flex items-center gap-3 rounded-xl border border-white/10 p-3 ${item.isActive ? "bg-white/[0.04]" : "bg-black/20 opacity-55"}`}>{item.photoUrl ? <img src={item.photoUrl} alt="" className="h-14 w-14 rounded-lg object-cover" /> : <span className="grid h-14 w-14 place-items-center rounded-lg bg-white/5"><List size={20} /></span>}<div className="min-w-0 flex-1"><strong className="block truncate">{item.name}</strong><p className="text-sm text-gold">{item.priceAzn.toFixed(2)} AZN</p></div><button onClick={() => setEditing(item)} title="Edit"><Pencil size={17} /></button><button onClick={() => void toggleItem(item)} className={item.isActive ? "text-red-300" : "text-emerald-300"}>{item.isActive ? "Deactivate" : "Activate"}</button></article>)}</div></div> : null; })}{!venueItems.length && <Gate title="No menu items yet" subtitle="Add one manually or paste multiple lines into the review table." />}</div>
...
function DealForm({ venues, categories, menuItems, editing, onOpenMenu, onClose, onSaved }: { venues: ManagedVenue[]; categories: MenuCategory[]; menuItems: MenuItem[]; editing: MerchantDeal | null; onOpenMenu: () => void; onClose: () => void; onSaved: () => void }) {
  ...
  return <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/70 p-4 backdrop-blur-sm"><form onSubmit={submit} className="mx-auto my-4 max-w-2xl rounded-xl border border-white/10 bg-[#12121a] p-5">
    ...
    <div className="grid gap-3 md:grid-cols-2">
      ...
      {selectedItemPhoto ? <div className="md:col-span-2 rounded-xl border border-emerald-300/20 bg-emerald-300/10 p-3"><span className="form-label text-emerald-200">Offer photo</span><div className="flex items-center gap-3"><img src={selectedItemPhoto} alt="Selected menu item" className="h-20 w-24 rounded-lg object-cover" /><p className="text-sm text-emerald-100">The selected menu item's saved photo will be used automatically. No additional upload is needed.</p></div></div> : <label className="md:col-span-2"><span className="form-label">Offer photo</span>{scope === "SPECIFIC_ITEMS" && selectedMenuItems.length > 0 && <p className="mb-2 rounded-lg border border-amber-300/20 bg-amber-300/10 p-2 text-sm text-amber-100">{selectedMenuItems.map((item) => item.name).join(", ")} {selectedMenuItems.length === 1 ? "does" : "do"} not have a saved menu photo. <button type="button" onClick={onOpenMenu} className="font-bold underline">Add the photo in Menu</button> to reuse it automatically.</p>}<input type="file" accept="image/jpeg,image/png,image/webp" className="form-field" onChange={(event) => { const file = event.target.files?.[0]; if (file) void readImage(file).then(setPhotoUrl).catch((reason) => setFormError(reason.message)); }} />{photoUrl && <img src={photoUrl} alt="Offer preview" className="mt-2 h-32 w-48 rounded-lg object-cover" />}<p className="mt-1 text-xs text-white/45">Required when none of the covered menu items has a saved photo.</p></label>}
