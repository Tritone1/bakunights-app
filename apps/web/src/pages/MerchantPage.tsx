...
type MerchantDeal = {
  ...
  scope: OfferScope; scopeCategoryId?: string | null; offerMenuItems: { menuItemId: string; overridePriceAzn?: string | number | null; quantity?: number | null; menuItem: MenuItem }[];
...
function DealForm({ venues, categoryOptions, menuItems, editing, onOpenMenu, onClose, onSaved }: { venues: ManagedVenue[]; categoryOptions: Record<string, VenueMenuCategoryOptions>; menuItems: MenuItem[]; editing: MerchantDeal | null; onOpenMenu: () => void; onClose: () => void; onSaved: () => void }) {
  ...
  const freeItemPickerId = selectedMenuItems.find((item) => itemOverrides[item.id] === "0")?.id ?? "";
  ...
  const offerTotal = selectedMenuItems.reduce((sum, item) => sum + (itemOverrides[item.id] === undefined || itemOverrides[item.id] === "" ? item.priceAzn : Number(itemOverrides[item.id])) * (itemQuantities[item.id] ?? 1), 0);
  ...
  return <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/70 p-4 backdrop-blur-sm"><form onSubmit={submit} className="mx-auto my-4 max-w-2xl rounded-xl border border-white/10 bg-[#12121a] p-5">
    ...
    <div className="grid gap-3 md:grid-cols-2">
      ...
      {scope === "SPECIFIC_ITEMS" && <div className="md:col-span-2 rounded-xl border border-white/10 p-3">
        ...
        {activeItems.length ? <><input value={itemSearch} onChange={(event) => setItemSearch(event.target.value)} className="form-field mb-2" placeholder={`Search items for this ${typeConfig.label.toLowerCase()}...`} /><div className="max-h-60 space-y-1 overflow-y-auto">{activeItems.filter((item) => item.name.toLowerCase().includes(itemSearch.toLowerCase())).map((item) => <div key={item.id} className="flex flex-wrap items-center gap-2 rounded-lg p-2 hover:bg-white/5"><label className="flex min-w-0 flex-1 items-center gap-2"><input type="checkbox" checked={selectedItems.includes(item.id)} onChange={() => { setSelectedItems((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : [...ids, item.id]); if (isSpendReward && freeMenuItemId === item.id) { setFreeMenuItemId(""); setFreeMenuItemQty(1); } }} /><span className="flex-1 truncate">{item.name}</span>{item.photoUrl && <span className="text-xs text-cyan-300">photo</span>}<span className="text-gold">{item.priceAzn.toFixed(2)} AZN</span></label>{selectedItems.includes(item.id) && showQuantity && <div className="flex items-center gap-1"><label className="text-xs text-white/40" htmlFor={`qty-${item.id}`}>Qty</label><input id={`qty-${item.id}`} aria-label={`Quantity for ${item.name}`} value={itemQuantities[item.id] ?? 1} onChange={(event) => { const next = Math.max(1, Math.min(20, Math.round(Number(event.target.value)) || 1)); setItemQuantities((values) => ({ ...values, [item.id]: next })); }} type="number" min="1" max="20" step="1" className="w-14 rounded-lg border border-white/10 bg-black/20 px-2 py-1 text-sm" /></div>}{selectedItems.includes(item.id) && showReprice && <div className="flex items-center gap-1.5"><input aria-label={`Reprice ${item.name}`} value={itemOverrides[item.id] ?? ""} onChange={(event) => setItemOverrides((values) => ({ ...values, [item.id]: event.target.value }))} type="number" min="0" step="0.01" className="w-28 rounded-lg border border-white/10 bg-black/20 px-2 py-1 text-sm" placeholder="New price" /><button type="button" onClick={() => setItemOverrides((values) => { const next = { ...values }; if (next[item.id] === "0") delete next[item.id]; else next[item.id] = "0"; return next; })} className={`rounded-lg border px-2 py-1 text-xs font-bold ${itemOverrides[item.id] === "0" ? "border-emerald-400/50 bg-emerald-400/15 text-emerald-200" : "border-white/10 text-white/55"}`}>Free</button></div>}</div>)}</div>{showReprice && selectedMenuItems.length > 0 && <div className="mt-3 rounded-lg bg-black/20 p-3 text-sm"><div className="flex justify-between text-white/55"><span>Regular total</span><span>{regularTotal.toFixed(2)} AZN</span></div><div className="mt-1 flex justify-between font-bold text-emerald-200"><span>Bundle total</span><span>{offerTotal.toFixed(2)} AZN</span></div>{savingsAmount > 0 && <div className="mt-1 flex justify-between text-amber-200"><span>Total saving</span><span>{savingsAmount.toFixed(2)} AZN ({savingsPercent}%)</span></div>}</div>}</> : <p className="text-sm text-white/55">No active items yet. <button type="button" onClick={onOpenMenu} className="font-bold text-cyan-300 underline">Add menu items</button> before creating this offer.</p>}
      </div>}
      {usesItemPrices && selectedMenuItems.length > 0 && <div className="md:col-span-2 rounded-xl border border-white/10 bg-white/[0.03] p-3"><label><span className="mb-1 block text-xs text-white/50">Customer receives free</span><select value={freeItemPickerId} onChange={(event) => { const value = event.target.value; setItemOverrides((values) => { const next = { ...values }; for (const item of selectedMenuItems) { if (next[item.id] === "0") delete next[item.id]; } if (value) next[value] = "0"; return next; }); }} className="form-field"><option value="">Choose the free item</option>{selectedMenuItems.map((item) => <option key={item.id} value={item.id}>{item.name} — {item.priceAzn.toFixed(2)} AZN</option>)}</select><span className="mt-1 block text-xs text-white/40">Shortcut for the Free button above — picking here zeroes that item's price.</span></label></div>}
