...
type MerchantDeal = {
  id: string; restaurantId: string; title: string; description: string; menuItem?: string | null; photoUrl?: string | null; offerType: OfferType; discountPct: number | null; offerPriceAzn?: string | number | null; minimumSpendAzn?: string | number | null; freeMenuItemId?: string | null; freeMenuItemQty?: number | null; isFlash: boolean; tag: string; dietaryTags: string[]; startsAt: string; endsAt: string; isActive: boolean; status: "draft" | "pending_review" | "approved" | "rejected" | "expired"; reviewNotes?: string | null;
...
function DealForm({ venues, categoryOptions, menuItems, editing, onOpenMenu, onClose, onSaved }: { venues: ManagedVenue[]; categoryOptions: Record<string, VenueMenuCategoryOptions>; menuItems: MenuItem[]; editing: MerchantDeal | null; onOpenMenu: () => void; onClose: () => void; onSaved: () => void }) {
  ...
  const [itemQuantities, setItemQuantities] = useState<Record<string, number>>(Object.fromEntries((editing?.offerMenuItems ?? []).filter((item) => (item.quantity ?? 1) > 1).map((item) => [item.menuItemId, item.quantity!])));
  const [itemFreeQuantities, setItemFreeQuantities] = useState<Record<string, number>>(Object.fromEntries((editing?.offerMenuItems ?? []).filter((item) => (item.freeQuantity ?? 0) > 0).map((item) => [item.menuItemId, item.freeQuantity!])));
  ...
  const [freeMenuItemQty, setFreeMenuItemQty] = useState(editing?.freeMenuItemQty ?? 1);
  ...
  const showQuantity = usesItemPrices || isSpendReward;
  ...
  const freeItemPickerId = selectedMenuItems.find((item) => (itemFreeQuantities[item.id] ?? 0) > 0)?.id ?? "";
  const selectedItemPhotos = [...new Map([...scopedItemPhotos, ...(freeMenuItem?.photoUrl ? [freeMenuItem] : [])].map((item) => [item.id, item])).values()];
  const regularTotal = selectedMenuItems.reduce((sum, item) => sum + item.priceAzn * (itemQuantities[item.id] ?? 1), 0);
  const offerTotal = selectedMenuItems.reduce((sum, item) => sum + (itemOverrides[item.id] === undefined || itemOverrides[item.id] === "" ? item.priceAzn : Number(itemOverrides[item.id])) * Math.max(0, (itemQuantities[item.id] ?? 1) - (itemFreeQuantities[item.id] ?? 0)), 0);
  ...
  useEffect(() => {
    ...
    const total = selectedMenuItems.reduce((sum, item) => {
      const qty = itemQuantities[item.id] ?? 1;
      const billableQty = item.id === freeMenuItemId ? Math.max(0, qty - freeMenuItemQty) : qty;
      return sum + item.priceAzn * billableQty;
    }, 0);
    if (total > 0) setMinimumSpend(String(total));
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [isSpendReward, scope, selectedItems, freeMenuItemId, itemQuantities, freeMenuItemQty]);
  useEffect(() => {
    if (!freeMenuItemId) return;
    const max = itemQuantities[freeMenuItemId] ?? 1;
    if (freeMenuItemQty > max) setFreeMenuItemQty(max);
    // eslint-disable-next-line react-hooks/exhaustive-deps
  }, [freeMenuItemId, itemQuantities]);
  useEffect(() => {
    if (!usesItemPrices) return;
    setItemFreeQuantities((values) => {
      let changed = false;
      const next: Record<string, number> = {};
      for (const [id, qty] of Object.entries(values)) {
        const max = itemQuantities[id] ?? 1;
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    const body = { restaurantId: String(form.get("restaurantId")), scope, scopeCategoryId: scope === "CATEGORY" ? scopeCategoryId : null, menuItemIds: scope === "SPECIFIC_ITEMS" ? selectedItems : [], menuItemOverrides: usesItemPrices ? Object.fromEntries(Object.entries(itemOverrides).filter(([id, value]) => selectedItems.includes(id) && value !== "").map(([id, value]) => [id, Number(value)])) : {}, menuItemQuantities: showQuantity ? Object.fromEntries(selectedItems.filter((id) => (itemQuantities[id] ?? 1) > 1).map((id) => [id, itemQuantities[id]])) : {}, menuItemFreeQuantities: usesItemPrices ? Object.fromEntries(selectedItems.filter((id) => (itemFreeQuantities[id] ?? 0) > 0).map((id) => [id, itemFreeQuantities[id]])) : {}, offerPriceAzn: typeConfig.usesTotalPrice ? numericOfferPrice : usesItemPrices ? offerTotal : null, minimumSpendAzn: isSpendReward ? Number(minimumSpend) : null, freeMenuItemId: isSpendReward ? freeMenuItemId : null, freeMenuItemQty: isSpendReward ? Math.max(1, Math.round(freeMenuItemQty || 1)) : 1, photoUrl: selectedItemPhotos.length ? null : photoUrl || null, title: String(form.get("title")), description: String(form.get("description")), menuItem: menuItem || null, offerType: String(form.get("offerType")), discountPct: discountValue ? Number(discountValue) : null, isFlash: flashCapableType && isFlash, tag, dietaryTags: typeConfig.showDietaryTags ? String(form.get("dietaryTags") || "").split(",").map((item) => item.trim()).filter(Boolean) : [], startsAt: new Date(startsAt).toISOString(), endsAt: new Date(endsAt).toISOString(), isRecurring: false };
  ...
  return <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/70 p-4 backdrop-blur-sm"><form onSubmit={submit} className="mx-auto my-4 max-w-2xl rounded-xl border border-white/10 bg-[#12121a] p-5">
    ...
    <div className="grid gap-3 md:grid-cols-2">
      ...
      {offerType === "bundle" && <fieldset className="md:col-span-2"><legend className="form-label">Bundle style</legend><div className="grid gap-2 sm:grid-cols-2"><button type="button" onClick={() => { setBundleRewardMode(false); setMinimumSpend(""); setFreeMenuItemId(""); setFreeMenuItemQty(1); setItemOverrides({}); setItemQuantities({}); setItemFreeQuantities({}); setFormError(""); }} className={`rounded-xl border p-3 text-left transition ${!bundleRewardMode ? "border-gold/50 bg-gold/10 text-amber-100" : "border-white/10 bg-white/[0.025] text-white/60"}`}><strong className="block text-sm">Fixed bundle</strong><span className="mt-1 block text-xs">Set each included item’s offer price or mark it free.</span></button><button type="button" onClick={() => { setBundleRewardMode(true); setItemOverrides({}); setItemQuantities({}); setOfferPrice(""); setFormError(""); }} className={`rounded-xl border p-3 text-left transition ${bundleRewardMode ? "border-gold/50 bg-gold/10 text-amber-100" : "border-white/10 bg-white/[0.025] text-white/60"}`}><strong className="block text-sm">Spend & get one free</strong><span className="mt-1 block text-xs">Example: spend 30 AZN on pizza and get one pizza free.</span></button></div></fieldset>}
      {offerType !== "event" && <label><span className="form-label">What does it cover?</span><select value={scope} disabled={typeConfig.allowedScopes.length === 1} onChange={(event) => { setScope(event.target.value as OfferScope); setScopeCategoryId(""); setSelectedItems([]); setItemOverrides({}); setItemQuantities({}); setItemFreeQuantities({}); setFreeMenuItemId(""); setFreeMenuItemQty(1); }} className="form-field disabled:cursor-not-allowed disabled:opacity-75">{typeConfig.allowedScopes.includes("WHOLE_MENU") && <option value="WHOLE_MENU">Whole menu</option>}{typeConfig.allowedScopes.includes("CATEGORY") && <option value="CATEGORY">One menu category</option>}{typeConfig.allowedScopes.includes("SPECIFIC_ITEMS") && <option value="SPECIFIC_ITEMS">Selected menu items</option>}</select>{typeConfig.allowedScopes.length === 1 && <span className="mt-1 block text-xs text-cyan-200">{isSpendReward ? "Choose the item the customer must buy." : `${typeConfig.label} offers require at least ${minimumItems} specific menu items.`}</span>}</label>}
      ...
      {scope === "SPECIFIC_ITEMS" && <div className="md:col-span-2 rounded-xl border border-white/10 p-3">
        ...
        {activeItems.length ? <><input value={itemSearch} onChange={(event) => setItemSearch(event.target.value)} className="form-field mb-2" placeholder={`Search items for this ${typeConfig.label.toLowerCase()}...`} /><div className="max-h-60 space-y-1 overflow-y-auto">{activeItems.filter((item) => item.name.toLowerCase().includes(itemSearch.toLowerCase())).map((item) => <div key={item.id} className="flex flex-wrap items-center gap-2 rounded-lg p-2 hover:bg-white/5"><label className="flex min-w-0 flex-1 items-center gap-2"><input type="checkbox" checked={selectedItems.includes(item.id)} onChange={() => { setSelectedItems((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : [...ids, item.id]); if (isSpendReward && freeMenuItemId === item.id) { setFreeMenuItemId(""); setFreeMenuItemQty(1); } if (usesItemPrices) setItemFreeQuantities((values) => { const next = { ...values }; delete next[item.id]; return next; }); }} /><span className="flex-1 truncate">{item.name}</span>{item.photoUrl && <span className="text-xs text-cyan-300">photo</span>}<span className="text-gold">{item.priceAzn.toFixed(2)} AZN</span></label>{selectedItems.includes(item.id) && showQuantity && <div className="flex items-center gap-1"><label className="text-xs text-white/40" htmlFor={`qty-${item.id}`}>Qty</label><input id={`qty-${item.id}`} aria-label={`Quantity for ${item.name}`} value={itemQuantities[item.id] ?? 1} onChange={(event) => { const next = Math.max(1, Math.min(20, Math.round(Number(event.target.value)) || 1)); setItemQuantities((values) => ({ ...values, [item.id]: next })); }} type="number" min="1" max="20" step="1" className="w-14 rounded-lg border border-white/10 bg-black/20 px-2 py-1 text-sm" /></div>}{selectedItems.includes(item.id) && showReprice && <div className="flex items-center gap-1.5"><input aria-label={`Reprice ${item.name}`} value={itemOverrides[item.id] ?? ""} onChange={(event) => setItemOverrides((values) => ({ ...values, [item.id]: event.target.value }))} type="number" min="0" step="0.01" className="w-28 rounded-lg border border-white/10 bg-black/20 px-2 py-1 text-sm" placeholder="New price" /><button type="button" onClick={() => setItemFreeQuantities((values) => { const next = { ...values }; if ((next[item.id] ?? 0) > 0) delete next[item.id]; else next[item.id] = 1; return next; })} className={`rounded-lg border px-2 py-1 text-xs font-bold ${(itemFreeQuantities[item.id] ?? 0) > 0 ? "border-emerald-400/50 bg-emerald-400/15 text-emerald-200" : "border-white/10 text-white/55"}`}>Free</button></div>}</div>)}</div>{showReprice && selectedMenuItems.length > 0 && <div className="mt-3 rounded-lg bg-black/20 p-3 text-sm"><div className="flex justify-between text-white/55"><span>Regular total</span><span>{regularTotal.toFixed(2)} AZN</span></div><div className="mt-1 flex justify-between font-bold text-emerald-200"><span>Bundle total</span><span>{offerTotal.toFixed(2)} AZN</span></div>{savingsAmount > 0 && <div className="mt-1 flex justify-between text-amber-200"><span>Total saving</span><span>{savingsAmount.toFixed(2)} AZN ({savingsPercent}%)</span></div>}</div>}</> : <p className="text-sm text-white/55">No active items yet. <button type="button" onClick={onOpenMenu} className="font-bold text-cyan-300 underline">Add menu items</button> before creating this offer.</p>}
      </div>}
      {usesItemPrices && selectedMenuItems.length > 0 && <div className="md:col-span-2 rounded-xl border border-emerald-300/20 bg-emerald-300/[0.06] p-3"><p className="form-label text-emerald-200">Free item in this fixed bundle</p><div className="grid gap-3 sm:grid-cols-2"><label><span className="mb-1 block text-xs text-white/50">Customer receives free</span><select value={freeItemPickerId} onChange={(event) => { const value = event.target.value; setItemFreeQuantities((values) => { const next = { ...values }; for (const item of selectedMenuItems) delete next[item.id]; if (value) next[value] = 1; return next; }); }} className="form-field"><option value="">Choose the free item</option>{selectedMenuItems.map((item) => <option key={item.id} value={item.id}>{item.name} — {item.priceAzn.toFixed(2)} AZN</option>)}</select><span className="mt-1 block text-xs text-white/40">Choose from the items included in this bundle.</span></label><label><span className="mb-1 block text-xs text-white/50">How many free</span><div className="flex items-center gap-2"><input aria-label="How many of the free item" value={freeItemPickerId ? itemFreeQuantities[freeItemPickerId] ?? 1 : 1} onChange={(event) => { if (!freeItemPickerId) return; const max = itemQuantities[freeItemPickerId] ?? 1; const next = Math.max(1, Math.min(max, Math.round(Number(event.target.value)) || 1)); setItemFreeQuantities((values) => ({ ...values, [freeItemPickerId]: next })); }} disabled={!freeItemPickerId} type="number" min="1" max={freeItemPickerId ? itemQuantities[freeItemPickerId] ?? 1 : 1} step="1" className="form-field disabled:cursor-not-allowed disabled:opacity-45" /><span className="shrink-0 text-xs text-white/50">of {freeItemPickerId ? itemQuantities[freeItemPickerId] ?? 1 : 0}</span></div><span className="mt-1 block text-xs text-white/40">The free quantity cannot exceed the included quantity.</span></label></div>{freeItemPickerId && <p className="mt-3 rounded-lg bg-black/20 p-3 text-sm font-semibold text-emerald-100">This fixed bundle includes {itemFreeQuantities[freeItemPickerId] ?? 1} of {selectedMenuItems.find((item) => item.id === freeItemPickerId)?.name ?? "the selected item"} free.</p>}</div>}
