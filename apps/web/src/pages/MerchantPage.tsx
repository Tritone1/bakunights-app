...
const OFFER_TYPE_CONFIG: Record<OfferType, {
  ...
  combo: { label: "Combo", description: "Combine two or more specific menu items into one offer.", allowedScopes: ["SPECIFIC_ITEMS"], minimumItems: 2, itemLabel: "Items in this combo", titleLabel: "Combo name", titlePlaceholder: "Burger lunch combo", descriptionPlaceholder: "Describe what is included in the combo and its total offer price.", usesItemPrices: true, showDietaryTags: true },
  set_menu: { label: "Set menu", description: "Create a fixed multi-item menu, such as starter, main, and dessert.", allowedScopes: ["SPECIFIC_ITEMS"], minimumItems: 2, itemLabel: "Courses in this set menu", titleLabel: "Set menu name", titlePlaceholder: "Three-course dinner", descriptionPlaceholder: "List the courses, set price, and any choices or conditions.", usesItemPrices: true, showDietaryTags: true },
  ...
  bundle: { label: "Bundle", description: "Package two or more items for a group, family, or larger order.", allowedScopes: ["SPECIFIC_ITEMS"], minimumItems: 2, itemLabel: "Items in this bundle", titleLabel: "Bundle name", titlePlaceholder: "Family dinner bundle", descriptionPlaceholder: "Describe the bundle contents, quantity, group size, and total price.", usesItemPrices: true, showDietaryTags: true },
...
function DealForm({ venues, categoryOptions, menuItems, editing, onOpenMenu, onClose, onSaved }: { venues: ManagedVenue[]; categoryOptions: Record<string, VenueMenuCategoryOptions>; menuItems: MenuItem[]; editing: MerchantDeal | null; onOpenMenu: () => void; onClose: () => void; onSaved: () => void }) {
  ...
  async function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (scope === "SPECIFIC_ITEMS" && selectedItems.length < typeConfig.minimumItems) { setFormError(`Choose at least ${typeConfig.minimumItems} menu ${typeConfig.minimumItems === 1 ? "item" : "items"} for this ${typeConfig.label.toLowerCase()}.`); return; }
  ...
  return <div className="fixed inset-0 z-[100] overflow-y-auto bg-black/70 p-4 backdrop-blur-sm"><form onSubmit={submit} className="mx-auto my-4 max-w-2xl rounded-xl border border-white/10 bg-[#12121a] p-5">
    ...
    <div className="grid gap-3 md:grid-cols-2">
      ...
      {scope === "SPECIFIC_ITEMS" && <div className="md:col-span-2 rounded-xl border border-white/10 p-3"><div className="mb-2 flex items-end justify-between gap-3"><p className="form-label mb-0">{typeConfig.itemLabel}</p><span className="text-xs text-white/40">Choose at least {typeConfig.minimumItems}</span></div>{activeItems.length ? <><input value={itemSearch} onChange={(event) => setItemSearch(event.target.value)} className="form-field mb-2" placeholder={`Search items for this ${typeConfig.label.toLowerCase()}...`} /><div className="max-h-60 space-y-1 overflow-y-auto">{activeItems.filter((item) => item.name.toLowerCase().includes(itemSearch.toLowerCase())).map((item) => <div key={item.id} className="flex items-center gap-2 rounded-lg p-2 hover:bg-white/5"><label className="flex min-w-0 flex-1 items-center gap-2"><input type="checkbox" checked={selectedItems.includes(item.id)} onChange={() => setSelectedItems((ids) => ids.includes(item.id) ? ids.filter((id) => id !== item.id) : [...ids, item.id])} /><span className="flex-1 truncate">{item.name}</span>{item.photoUrl && <span className="text-xs text-cyan-300">photo</span>}<span className="text-gold">{item.priceAzn.toFixed(2)} AZN</span></label>{selectedItems.includes(item.id) && typeConfig.usesItemPrices && <input aria-label={`Offer price for ${item.name}`} value={itemOverrides[item.id] ?? ""} onChange={(event) => setItemOverrides((values) => ({ ...values, [item.id]: event.target.value }))} type="number" min="0.01" step="0.01" className="w-28 rounded-lg border border-white/10 bg-black/20 px-2 py-1 text-sm" placeholder="Offer AZN" />}</div>)}</div>{typeConfig.usesItemPrices && overriddenItems.length > 0 && <p className="mt-3 text-sm text-emerald-200">Combined offer price: <strong>{offerTotal.toFixed(2)} AZN</strong> <span className="text-white/40">(regularly {regularTotal.toFixed(2)} AZN)</span></p>}</> : <p className="text-sm text-white/55">No active items yet. <button type="button" onClick={onOpenMenu} className="font-bold text-cyan-300 underline">Add menu items</button> before creating this offer.</p>}</div>}
