...
type Enrollment = { id: string; venueName: string; venueType: string; venueAddress: string; venueLat: number; venueLng: number; contactPhone: string; contactEmail: string; proofNotes: string; status: "pending" | "approved" | "rejected"; reviewNotes?: string | null };
...
export function ProfilePage() {
  ...
  return <div className="min-h-screen bg-night px-4 pb-20 pt-24 text-white sm:px-6"><div className="mx-auto max-w-5xl">
    ...
    {showEnrollment && <form onSubmit={submitEnrollment} className="mt-6 grid gap-4 rounded-2xl border border-white/10 bg-white/[0.04] p-6 md:grid-cols-2"><div className="md:col-span-2"><p className="text-[10px] font-bold uppercase tracking-[.2em] text-gold">Merchant verification</p><h2 className="mt-1 font-display text-3xl">Tell us about your venue</h2></div><Field name="venueName" label="Venue name" defaultValue={enrollment?.venueName} /><Select name="venueType" label="Venue type" required defaultValue={enrollment?.venueType ?? ""} options={[{ value: "", label: "Choose venue type", disabled: true }, ...VENUE_TYPES]} /><Field name="contactPhone" label="Contact phone" defaultValue={enrollment?.contactPhone} /><Field name="contactEmail" label="Contact email" type="email" defaultValue={enrollment?.contactEmail || user.email} wide /><Field name="venueAddress" label="Full venue address" defaultValue={enrollment?.venueAddress} wide /><Field name="venueLat" label="Latitude" type="number" step="any" defaultValue={enrollment?.venueLat} /><Field name="venueLng" label="Longitude" type="number" step="any" defaultValue={enrollment?.venueLng} /><label className="md:col-span-2"><span className="form-label">Proof you manage this venue</span><textarea name="proofNotes" required minLength={20} className="form-field min-h-28" defaultValue={enrollment?.proofNotes} /></label><div className="flex justify-end gap-2 md:col-span-2"><button type="button" onClick={() => setShowEnrollment(false)} className="rounded-xl border border-white/10 px-4 py-2">Cancel</button><button disabled={Boolean(busy)} className="panel-button">Send for review</button></div></form>}
  </div>
