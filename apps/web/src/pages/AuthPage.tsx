    ...
    try {
      if (isRegister) {
        ...
        if (accountType === "MERCHANT") {
          ...
          signupData.set("venueType", String(form.get("venueType") || ""));
      ...
      <section className="flex items-center justify-center px-4 py-10 sm:px-8">
        <div className="w-full max-w-md">
          ...
          <form onSubmit={submit} className="mt-6 space-y-4">
            ...
            {isRegister && accountType === "MERCHANT" && <label className="block"><span className="form-label">Venue type</span><select className="form-field" name="venueType" required defaultValue=""><option value="" disabled>Choose venue type</option>{VENUE_TYPES.map((type) => <option key={type.value} value={type.value}>{type.label}</option>)}</select><span className="mt-1 block text-xs text-white/45">Choose the option that best describes your venue.</span></label>}
